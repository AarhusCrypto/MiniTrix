...
MATRIX * build_e() {
  ...
  byte omega = 1;
  MATRIX * small = new_matrix(120,120);
  MATRIX * big = new_matrix(120,255);
...
MATRIX * build_small_encoder(uint dim) {
  byte nroot = 0x03;
  MATRIX * m = 0, * small = 0, * res = 0;
  uint i=0,j=0;
  byte omega1 = 1, omega2 = nroot;
  byte val1 = 1, val2 = 1;
  m = new_matrix(dim,dim);
  for(i = 0;i<85;++i) { // row
    val1 = 1;val2 = 1;
 ...
 */
static 
MATRIX * build_combi_enc_matrix() {
  MATRIX * M = build_nth_matrix(255,120,3);
  MATRIX * m = build_nth_matrix(120,120,3);
  MATRIX * t1 =0 ,*t2 = 0;
  t1 = LUInverse(m);
