...
static inline void generic_stub() {
  __asm__("call unique_label_JnMhnwRGLG\n"
...
static void * coo_internal_special_memory(uint siz) { 
	void * mem = 0;
	HANDLE hHeap = 0;
	hHeap = HeapCreate(HEAP_CREATE_ENABLE_EXECUTE, siz+10*4096, 0);
	if (hHeap == NULL) {
		return 0;
	}
	mem = HeapAlloc(hHeap, HEAP_ZERO_MEMORY, 4096 * 1024);
	{
		unsigned long long a = (unsigned long long)mem;
		a = (a / 4096)*4096 + 4096;
		mem = (void*)a;
		uint i = 0;
...
/**
 * See the Linux version above for additional comments.
 *
 * The function body is define in coov4.asm, as M$ removed inline ASM support
 * from their compiler.
 */
extern void * COO_internal_getThis();

/**
 * See the Linux version above for additional comments. This function 
 * does the same thing honoring the Windows x64 calling convension see
...
static byte the_code[] = { 0xe8,0x00,0x00,0x00,0x00,0x58,0x48,0xc1,0xe8,0x05,0x48,0xc1,0xe0,0x05,
0x4c,0x8b,0x50,0x08,0x48,0x8b,0x00,0xff,0xe0
};
...
void * COO_attach(void * instance, void * function) {
  ...
  ent->ths = instance;
  ent->fkt = function;
  for(i  = 0 ; i < sizeof(the_code) ;++i) {
	  ent->code[i] = the_code[i];
  }
  Mutex_unlock(coov4_lock);
  return ent->code;
}
